"""Shared chassis Qt widgets.

This module is a widget library, not an app: the old standalone body-stub
window (`BodyStubWindow`/`QtUI`, launched via `python -m desktop.chassis`)
was deleted in the 2026-06 cleanup. What remains is reused by the nav app
and the sweep calibration dock:

  * VisionDock (+ _VisionWorker/_TTSWorker/_ChatInput) — camera/VLM chat
  * MotorTestDock, DifferentialPad — teleop / motor bring-up
  * LocalMapView, DriveableView — body-frame map views (sweep_dock)
  * depth_to_pixmap / _overlay_boxes — depth + detection rendering
...
from PyQt6.QtCore import Qt, QPointF, QProcess, QThread, pyqtSignal
