...
- **`desktop/`** — Operator-side stack (laptop / workstation). Apps: **`mapping`** builds a reference map, **`nav`** drives against it (manual teleop **plus** autonomous hierarchical Tier-1/2/3 — the main operator UI, and the `heartbeat`/`cmd_vel` source for standalone runs), **`map_editor`** cleans the map (occupancy + no-go layers, no drive controls); debug tools are **`pi_drive`** (Tier-2/3 console), **`handoff_inspector`** (breakpoint + visualize each Tier-1→2→3 handoff), and **`nav.drive_timing_logger`** (capture/analyze drive-timing for smoothness tuning). (`localization`, `reference_map`, `world_map`, `chassis` are supporting libraries, not apps.) All connect to the Pi over the same Zenoh router.
...
**Pose provider:** the hierarchical drive runs on the PF posterior by default; **`--checkpoint-pose`** switches to odom dead-reckoning + checkpoint re-anchor (needs Recognized checkpoints in the map — see [Map editing](#map-editing-desktopmap_editor)). On this map class it's the robust choice — plain MCL can lose orientation in low-feature corridors. The dead-reckoned yaw comes from the **IMU**, not the wheels (a floor-ridge kick rotates the chassis without the wheels seeing it; the prior follows the IMU so re-anchoring keeps matching). Watch the nav console: `checkpoint: re-anchor …` lines show each snap and the drift it corrected; `checkpoint: re-anchor FAILING — N consecutive rejected attempts, X m dead-reckoned …` means the pose is free-running where it expected an anchor.
...
- **Tier 2** (desktop) — **tracks** that route rather than beelining: advances a carrot one sub-waypoint at a time via a *passed-vertex* test (never skipping closely-spaced corner points), and hands Tier-3 the furthest live-clear point along that bearing, marched on **Tier-3's own footprint-inflated costmap** so the sub-goal is one Tier-3 accepts without snapping. It re-picks the sub-goal when the current one is reached, when the bearing to the carrot drifts (wrap-safe across ±π), or when the localizer applies a **discrete pose correction** (operator relocate, checkpoint re-anchor, or a PF posterior jump past the discrete gates — *not* every routine scan observation) — so the heading tracks a bump the odom frame never saw, instead of waiting out the leg. The **terminal** waypoint is reached by the same passed-vertex test (driven *up to* the endpoint), not a bare proximity radius, so dense routes don't stop a leg short. The loop runs on its own fast timer (decoupled from UI redraw), so the next sub-goal is sent within a tick of the last one being reached — the drive doesn't stutter between sub-goals.
- **Tier 3** (Pi, `body.local_drive`) — the single local-routing authority: inflates the live scan into a costmap, runs local A\*, follows with pure-pursuit, and a swept-footprint veto guards each arc; owns `body/cmd_vel` (and **commands zero immediately on cancel/stop** — no coasting out the motor timeout). On a swept-block it **re-aims in place** (a pure rotation is collision-free for the circular footprint) to straighten the approach instead of stopping dead. A **per-goal deadline** (`goal_deadline_s`) backstops the no-progress watchdog so a rotate/drive dither can't keep a goal alive forever. The goal's body-frame bearing uses an **IMU-corrected heading** — wheel θ plus the IMU-vs-wheel yaw divergence since the goal started (per-goal baseline, so IMU drift can't enter; no IMU → wheel-only): wheel odom is blind to a floor ridge kicking the chassis, and without this the follower arcs off-route by exactly the missed angle.

**Blocked handling is time-based:** on a Tier-3 BLOCKED, the desktop retries selection at `blocked_retry_interval_s` for `blocked_retry_window_s` (so a person walking through the lidar clears on their own), then pauses; the toolbar **Resume** restarts a BLOCKED pause or a SUSPENDED (connectivity) hold — the robot never lurches back into motion on its own.

**One config source (contract I8):** the desktop models Tier-3 (the Tier-2 clear-run, the debug consoles) with planner/raster configs built from `config.json` by the same `body/lib/drive_config.py` builders the Pi uses — the two sides cannot drift apart silently, and `body/test_drive_config.py` rejects dead config keys.
...
A read-only diagnostic for tuning the drive's smoothness. It subscribes to `body/drive/goto` + `body/drive/status`, captures a patrol to JSONL, and on Ctrl-C reports the per-sub-goal **handoff gap** (sub-goal ARRIVED → next goto, split into desktop-side vs link/Pi), the commanded **speed-vs-distance ramp**, and the **leg lengths** — the numbers behind the `slowdown_distance_m` / re-pick-latency tuning. Re-analyze an existing capture with `--analyze`.

```bash
desktop/.venv/bin/python -m desktop.nav.drive_timing_logger --router tcp/PI_IP:7447
# ...drive a patrol, Ctrl-C to stop + summarize. Re-read a capture:
desktop/.venv/bin/python -m desktop.nav.drive_timing_logger --analyze drive_timing_*.jsonl
```
...
- **Occupancy** — paint **Wall / Free / Unknown** with a disk brush. This is the *perception* layer: it feeds **both** MCL localization (the occupancy evidence the scan matcher scores against) and planning. Note nav treats **Unknown as blocked** (lethal) and the map is static (never written back at runtime), so **paint Free along any corridor you want patrols to drive** — nav never converts unknown→free on its own.
...
Inspect or erase the map's LPR checkpoints from the CLI with
`python -m desktop.localization.checkpoint_tool {list,delete,clear} MAP.npz`
(metadata-only; writes a `.bak` unless `--no-backup`).
...
- [body/](body/) — Pi-side package: `launcher`, drivers (`motor_controller`, `lidar_driver`, `oakd_driver`, `watchdog`, `imu_driver`), `local_map`, `local_drive` (Tier-3 reactive drive), `lib/` (`zenoh_helpers`, `schemas`, `diff_drive`, `host_metrics`, `drive_config` (config.json → planner/raster configs, shared with the desktop), and the pure drive cores `astar`, `local_costmap`, `local_planner`, `scan_raster`, `drive_safety`, `tier2_subgoal`).
- [desktop/](desktop/) — operator-side packages. **Apps:** [`mapping`](desktop/mapping) (reference-map builder + teleop), [`nav`](desktop/nav) (operator UI: teleop + hierarchical drive), [`map_editor`](desktop/map_editor) (reference-map editor: occupancy + no-go layers), [`pi_drive`](desktop/pi_drive) (Tier-2/3 debug console), [`handoff_inspector`](desktop/handoff_inspector) (breakpoint/visualize the Tier-1→2→3 handoffs), [`nav.drive_timing_logger`](desktop/nav/drive_timing_logger.py) (capture/analyze drive-timing). **Libraries:** [`reference_map`](desktop/reference_map) (frozen-map I/O + likelihood/distance fields), [`localization`](desktop/localization) (MCL + checkpoint re-anchor), [`world_map`](desktop/world_map) (shared costmap / grid / particle-filter — fuser app retired), [`chassis`](desktop/chassis) (zenoh transport/controller + the teleop/vision widget library reused by nav; the standalone chassis app was deleted 2026-06 — see [desktop/CLEANUP.md](desktop/CLEANUP.md)), `vision_service.py`, `utils/`.
