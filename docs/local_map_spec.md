...
**Driveable grid (optional):** When `driveable_enabled` is true, the message may include `driveable` (same shape as `max_height_m`) with `true` / `false` / `null` per cell. A cell is **blocked** if lidar or depth places **any** sample whose **height above the fixed horizontal floor plane** lies between `driveable_floor_band_m` and `driveable_clearance_height_m` (exclusive of the floor band, inclusive of clearance top). Returns above the clearance height are ignored for driveability. Until `driveable_clear_frames` consecutive observed frames have no slab hit, a cell stays **not** driveable (sticky clear). Unobserved cells keep the previous driveable verdict.
