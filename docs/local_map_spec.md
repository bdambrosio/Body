...
**Driveable grid (optional):** When `driveable_enabled` is true, the message may include `driveable` (same shape as `max_height_m`) with `true` / `false` / `null` per cell. A cell is **blocked** when samples whose **height above the fixed horizontal floor plane** lies between `driveable_floor_band_m` and `driveable_clearance_height_m` (exclusive of the floor band, inclusive of clearance top) trigger the obstacle/slab logic: lidar uses `lidar_slab_block_frames` consecutive frames with at least `lidar_slab_min_hits` slab hits per frame; depth uses `depth_slab_block_frames` consecutive frames with at least `driveable_slab_min_pixels` slab pixels per frame. Returns above the clearance height are ignored for driveability. Until `driveable_clear_frames` consecutive observed frames have no slab hit, a cell stays **not** driveable (sticky clear). Unobserved cells publish `null`.
...
| `driveable_slab_min_pixels` | Minimum depth pixels in a cell whose plane-relative height lands in the obstacle slab before the depth slab streak can advance this frame. **1** = any speckle counts. Default **2**. |
| `depth_slab_block_frames` | Consecutive frames that meet the depth slab pixel threshold before depth can declare a slab block for that cell (default **2**; mirrors lidar `lidar_slab_block_frames`). Set **1** for prior single-frame depth blocking. |
...
| `floor_fit_enabled` | Optional diagnostic RANSAC floor fit. Disabled by default; driveability uses the fixed horizontal floor plane. |
| `floor_fit_interval_s` | Minimum time between diagnostic RANSAC floor fits on depth when `floor_fit_enabled` is true. |
| `floor_fit_ransac_iters`, `floor_fit_inlier_m`, `floor_fit_min_inliers`, `floor_fit_max_samples` | Diagnostic RANSAC subsample and quality. |
| `floor_roi_u0`, `floor_roi_u1`, `floor_roi_v0`, `floor_roi_v1` | Normalized depth image ROI \([0,1]\) for diagnostic floor-fit points. |
| `floor_fit_log_interval_s` | Seconds between stdout logs of the diagnostic fitted plane (`n`, `d`, implied pitch / roll). Set **0** to disable. Use the printed `depth_pitch_rad` / `depth_roll_rad` suggestions only as rough calibration hints. |
